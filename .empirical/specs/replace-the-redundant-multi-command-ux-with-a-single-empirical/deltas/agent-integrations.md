...
Expose one coherent Empirical experience in every supported agent while keeping
package lifecycle operations explicit and safe.
...
The separate project-integration versus global-integration choice is removed
from the normal product surface. Global installation is always the explicit
`empirical install` lifecycle action, while agent-owned repository
initialization never writes user-level configuration.

#### Scenario: An agent initializes repository state

- **WHEN** the one installed Empirical entrypoint enters an uninitialized repository
- **THEN** it initializes repository state and context without installing global skills
- **AND** no separate project integration command is offered
